...
def split(project_file: str, data_file: str, output_dir: str, seed=42, split=0.8):
    random.seed(seed)
    ...
    # Split the issues
    n_issues = len(issues)
    training_issues = set(random.sample(list(issues), k=round(n_issues * split)))
...
def main():
    ...
    split(args.project_file, args.data_file, args.output_dir)
